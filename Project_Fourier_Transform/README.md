...
$$F(\omega ) = \int_{-\infty }^{\infty }f(t)e^{-jwt}dt \approx \sum_{n=0}^{N-1} x_{n}e^{-\frac{2\pi i}{N}kn} = \sum_{n=0}^{N-1} x_{n}(cos(\frac{2\pi kn}{N})-j*sin(\frac{2\pi kn}{N}))$$
