...
In the end, since both OpenMPI and OpenMP were used, MPI communicator initializer was changed to be the thread equivalent, which is MPI_Init_thread(), requesting to be MPI_THREAD_MULTIPLE, the processes quantity is a number sent by the user when running the code, internally into DFT code, since OpenMPI is taking the outer loop, it was needed to dimension the block that each rank will be taking, so that block size is $iSamplesQty/iRanksQty$, initial position is $iMyRank*iBlockSize$, and final position is $iInit + iBlockSize$.
